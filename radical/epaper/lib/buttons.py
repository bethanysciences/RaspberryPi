...
class Buttons(object):
    ...
    def __init__(self, keys, button_handler):
        ...
        GPIO.add_event_detect(keys[0], GPIO.FALLING,
            callback=lambda pin: self.button_pressed(1, button_handler),
            bouncetime=200)
        GPIO.add_event_detect(keys[1], GPIO.FALLING,
            callback=lambda pin: self.button_pressed(2, button_handler),
            bouncetime=200)
        GPIO.add_event_detect(keys[2], GPIO.FALLING,
            callback=lambda pin: self.button_pressed(3, button_handler),
            bouncetime=200)
        GPIO.add_event_detect(keys[3], GPIO.FALLING,
            callback=lambda pin: self.button_pressed(4, button_handler),
            bouncetime=200)
